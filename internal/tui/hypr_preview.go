...
func (h *HyprPreviewPane) View() string {
	...
	if len(h.monitors) == 0 {
		sections = append(sections, h.colors.HyprConfigTitleStyle().Render(
			"Hyprland Config Preview\n\nNo monitors to configure"))
	} else {
		sections = append(sections, h.colors.HyprConfigTitleStyle().Render("Hyprland Config Preview"))
	}
	...
	for _, monitor := range h.monitors {
		...
		sections = append(sections, h.colors.HyprCommandStyle().Render(hyprCommand))
	}
