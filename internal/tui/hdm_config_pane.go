...
func (h *HDMConfigPane) renderNoMatchingProfile() string {
	...
	sections = append(sections, h.colors.HyprConfigTitleStyle().Width(h.width).Margin(0, 0, 1, 0).Render("No Matching Profile"))
	...
	what := h.colors.ConfigPaneActionStyle().Render("Press ")
	action := h.colors.HeaderStyle().Render("'n'")
	rest := h.colors.ConfigPaneActionStyle().Render(" to create a new profile")
	actionBox := lipgloss.JoinHorizontal(lipgloss.Left, what, action, rest)
...
func (h *HDMConfigPane) renderMatchedProfile(profile *config.Profile) string {
	...
	result.WriteString(h.colors.HyprConfigTitleStyle().Render(
	...
	if !h.hasMonitorCountMismatch(profile) {
		...
		statusBox := h.colors.SubtitleInfoStyle().Width(h.width).
...
func (h *HDMConfigPane) renderMonitorMismatchWarning(profile *config.Profile) string {
	...
	return h.colors.ConfigPaneWarningStyle().Render(content.String())
}
...
func (h *HDMConfigPane) renderProfileDetails(profile *config.Profile) string {
	...
	content.WriteString(h.colors.ConfigDetailLabelStyle().Render("Config File: "))
	content.WriteString(h.colors.ConfigDetailStyle().Render(
		h.truncateString(filepath.Base(profile.ConfigFile), 40)))
	content.WriteString("\n")

	if profile.ConfigType != nil {
		content.WriteString(h.colors.ConfigDetailLabelStyle().Render("Config Type: "))
		content.WriteString(h.colors.ConfigDetailStyle().Render(
	...
	if profile.Conditions != nil && profile.Conditions.PowerState != nil {
		content.WriteString(h.colors.ConfigDetailLabelStyle().Render("Power State: "))
		content.WriteString(h.colors.ConfigDetailStyle().Render(profile.Conditions.PowerState.Value()))
		content.WriteString("\n")
	}

	if profile.Conditions != nil && profile.Conditions.LidState != nil {
		content.WriteString(h.colors.ConfigDetailLabelStyle().Render("Lid State: "))
		content.WriteString(h.colors.ConfigDetailStyle().Render(profile.Conditions.LidState.Value()))
		content.WriteString("\n")
	}

	if profile.Conditions != nil && profile.Conditions.RequiredMonitors != nil {
		content.WriteString(h.colors.ConfigDetailLabelStyle().Render("Required Monitors:"))
		content.WriteString("\n")
