...
func (m ModeItem) View() string {
	...
	if isExactStandard {
		styledSuffix = m.colors.StandardModeSuffixStyle().Render(" " + suffix)
	} else {
		styledSuffix = m.colors.NonStandardModeSuffixStyle().Render(" " + suffix)
	}
...
func (d ModeDelegate) Render(w io.Writer, m list.Model, index int, item list.Item) {
	...
	switch {
	case index == m.Index():
		style = d.colors.MonitorListSelected()
		prefix = "► "
	default:
		style = d.colors.MonitorListTitle()
	}
